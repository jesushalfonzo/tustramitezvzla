...
if((isset($_POST["correoUsuario"]))&&($_POST["correoUsuario"]!="")){ $correoUsuario=mysqli_real_escape_string($link, $_POST["correoUsuario"]); } else {$aErrores[] = "Debe indicar un email válido";}
...
if((isset($_POST["addressUser"]))&&($_POST["addressUser"]!="")){ $addressUser=mysqli_real_escape_string($link, $_POST["addressUser"]); } else {$aErrores[] = "Debe indicar la dirección --> $dir";}
...
if (!filter_var($correoUsuario, FILTER_VALIDATE_EMAIL)){
	$aErrores[] = "El correo $correoUsuario no tiene un formato válido";
} 
*/
...
$SQLVal="SELECT m_cliente_documentoIdentidad, m_cliente_email FROM m_clientes WHERE m_cliente_documentoIdentidad='$docIdentidad' OR m_cliente_email='$correoUsuario'";
$queryVal=mysqli_query($link, $SQLVal);
$total=mysqli_num_rows($queryVal);

if ($total >0) {
	$aErrores[] = "Este usuario ya se encuentra registrado $correoUsuario";
}
...
if(count($aErrores)==0) { 
	...
	VALUES (Null, '$nombre', '$apellido', '$tipoDocumento', '$docIdentidad', '$tipoTelefono', '$numtelefonico', '$correoUsuario', '$paswordEncriptado', '$country', '$addressUser', '$fechacompleta', '$fechacompleta', '0', '$hash')";
	...
	if ($resultado) {
		...
		$jsondata["data"] = array(
			'message' => "Ha sido registrado exitosamente,  en pocos minutos le esviaremos un correo a la dirección ($correoUsuario) para que confirme su registro!"
			);

		$nombreMail=$nombre." ".$apellido;
		sendMail($correoUsuario, $nombreMail , $hash);
	} else {
